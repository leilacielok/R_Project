...
# Drop Granade that I don't have in the world dataframe
```{r}
data <- drop_na(data)
sum(is.na(data))
```
...
# Check whether the difference is due to differences in the literacy rates among countries.Then, check wheter literacy rates may determine different IQ.
```{r}
Geo.literacyrate <- aov(Literacy.Rate ~ Continent, data = IQdataset)
summary(Geo.literacyrate)
```
...
#############################################################
## 2nd HYPOTHESIS: the IQ may depend on other qualities of life.

#ANOVA to see whether different geographical zones have different qualities of life.
```{r}
...
summary(Geo_Pop)
```
#Both HDI and GNI depend on the continent at the 1% level.
...
#First, I perform other anovas with the IQ level.
```{r}
...
```{r}
lm.IQ.GNI <-lm(Average.IQ ~ GNI...2021, data = IQdataset)
summary(lm.IQ.GNI)

lm.IQ.HDI <-lm(Average.IQ ~ HDI..2021., data = IQdataset)
summary(lm.IQ.HDI)

lm.IQ.POP <- lm(Average.IQ ~ HDI..2021. + GNI...2021 + Population...2023, data = IQdataset )
summary(lm.IQ.POP)
```
